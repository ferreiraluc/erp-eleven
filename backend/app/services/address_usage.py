...
def usage_query(ids):
    freight=select(FreightOrder.id.label('id'),literal('frete').label('kind'),FreightOrder.created_at.label('created_at'),
        ...
        FreightOrder.environment.label('environment')).join(Usuario,Usuario.id==FreightOrder.user_id).where(FreightOrder.address_id.in_(ids))
    printing=select(PrintJob.id,literal('impressao'),PrintJob.created_at,PrintJob.status,
        PrintJob.source,Usuario.nome,literal(None),literal(None),PrintJob.snapshot,literal(None)).join(
        Usuario,Usuario.id==PrintJob.user_id).where(PrintJob.address_id.in_(ids))
    return union_all(freight,printing).subquery()


def usage_stats(db, ids):
    q=usage_query(ids)
    row=db.execute(select(func.count().label('total'),func.max(q.c.created_at).label('last_used_at'),
        ...
        func.sum(case((q.c.kind=='impressao',1),else_=0)).label('prints'))).mappings().one()
    return {k:(v or 0) if k!='last_used_at' else v for k,v in row.items()}
...
def history(db, address, offset=0, limit=30, kind='all'):
    ...
    result=[]
    for item in db.execute(query.order_by(table.c.created_at.desc(),table.c.kind,table.c.id).offset(offset).limit(limit)).mappings():
        row=dict(item)
        row['id']=str(row['id'])
