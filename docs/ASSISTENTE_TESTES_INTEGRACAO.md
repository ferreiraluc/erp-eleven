...
- O proprietário confirmou o número com final 2052 e autorizou seu vínculo ao ADMIN do Telegram. A mensagem real das 21h45 foi recuperada pela API da Twilio e reenfileirada uma vez, mantendo o Message SID original para idempotência. O worker consultou o banco real, gerou os últimos cinco rastreios e marcou a entrada `done` (`afa73cc9-13c5-49c8-a909-5e53cc45fb04`).
- O envio da resposta falhou com `twilio_20003`, sem Message SID de saída. Diagnóstico reproduzido por SDK e REST, sem enviar novas mensagens: HTTP 401, **Primary compliance profile is not approved**, exigindo concluir o KYC no Trust Hub. As mesmas credenciais conseguem consultar mensagens e perfis. A lista de CustomerProfiles retornou zero perfis, sem próxima página. O número ONLINE e a conta Full não comprovam liberação de envios.
- [Twilio — erro 20003](https://www.twilio.com/docs/api/errors/20003) e [perfil principal do Trust Hub](https://www.twilio.com/docs/trust-hub/profiles/primary-compliance-profiles). A página do perfil foi aberta, mas solicitou login do proprietário. Não foram submetidos dados de identidade ou documentos, nem criados perfis de verificação com dados presumidos.
- Após a liberação externa, reprocessar somente a entrega com falha (evento `reply:afa73cc9-13c5-49c8-a909-5e53cc45fb04`) se a janela de atendimento ainda estiver aberta; se tiver expirado, solicitar nova mensagem. Não regenerar nem duplicar a entrada já processada.
- Seis números adicionais foram cadastrados e nomeados conforme o proprietário: Denis (4123), Wissam (4734, 8383, 5557), Sol (5736) e Juninho (3955). Acesso inicial de consulta, role VENDEDOR, `can_register=false`; o proprietário mantém ADMIN e registro habilitado. As três identidades de Wissam compartilham um usuário. Perfis provisórios sem atividade ficaram inativos. Nenhuma senha de login foi entregue e nenhum registro de vendedor/calendário foi alterado.
- Verificação direta das seis identidades confirmou autorização de consulta e bloqueio das ferramentas de vendas administrativas. Não foram enviadas mensagens de teste aos funcionários.
...
### Revalidação às 22h22 — novo cadastro empresarial informado
